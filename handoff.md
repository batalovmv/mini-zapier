...
- **Prod verification (Vercel `mini-zapier-web-silk.vercel.app`, 2026-03-16)**:
  ...
  - Template picker (`/workflows/new`): 3 карточки (Webhook→Telegram, Cron→Email, Blank Workflow) — ✅
...
- **Что сделано в TASK-J**:
  - `apps/web/e2e/ui-smoke.spec.ts` — добавлены `resolveE2eEmail()` + `ensureE2eUser()`: при отсутствии `MINI_ZAPIER_E2E_EMAIL` smoke генерирует валидный per-run email из legacy username, один раз создаёт пользователя через `POST /api/auth/register`, затем логинится уже по стабильному email/password; параллельно зафиксированы `mini-zapier:locale=en` и helper `waitForDashboard()` на основе `data-testid="create-workflow-link"` вместо hero-copy
...
| TASK-J | done | см. `git log` (`TASK-J: repair live E2E smoke auth and dashboard contract`) | Live smoke now self-provisions a valid e2e user when only legacy username is configured, forces EN locale, waits for stable dashboard markers, and CI also passes explicit `MINI_ZAPIER_E2E_EMAIL` |
