...
## Текущее состояние
- **Последнее изменение**: TASK-S3 — `RU locale visual polish — fix overflow and layout across all pages`
...
## Следующий шаг
Следующий рабочий срез: не назначен. TASK-S3 закрыт.
...
| TASK-R1 | done | см. `git log` (`TASK-R1: localize known backend error messages on frontend`) | Added `backendErrors` lookup map to both locale files and `getApiErrorMessage`; known backend strings (Invalid credentials, User already exists, etc.) now display in the active locale; unknown strings fall through as-is |
| TASK-R2 | done | см. `git log` (`TASK-R2: dashboard density — hide zero attention cards, move stats below list`) | Attention strip hides zero-count cards (compact "all clear" line when all zero), stats moved below workflow list grid; web build + Playwright test list passed |
| TASK-R3 | done | см. `git log` (`TASK-R3: compact editor node cards — remove description body, reduce size`) | Removed body section (duplicate type label + description) from ActionNode/TriggerNode; reduced min-w 240→200, padding px-4 py-3→px-3 py-2.5, border-radius rounded-3xl→rounded-xl; web build + Playwright test list passed |
| TASK-R4 | done | см. `git log` (`TASK-R4: reduce canvas header chrome to compact step counter`) | Replaced ~80px canvas header (eyebrow + title + description + inspector status) with compact 36px step counter pill; removed 10 dead locale keys from EN/RU; removed unused selectedNodeLabel useMemo + selectedNodeId selector; web build + Playwright test list passed |
| TASK-R5 | done | см. `git log` (`TASK-R5: collapsible editor toolbox with localStorage persistence`) | NodeSidebar toggle: collapsed 48px column with "+" button, expanded 300px as before; state persisted via localStorage key `mini-zapier:toolbox-collapsed`; default expanded; collapse/expand buttons in header and collapsed placeholder; dynamic grid-cols on WorkflowEditorPage; web build + Playwright test list passed |
| TASK-R6 | done | см. `git log` (`TASK-R6: widen inspector to 380px when toolbox collapsed`) | Inspector column grows from 320px to 380px (400px on 2xl) when toolbox collapsed; expanded state unchanged; single grid-cols line change in WorkflowEditorPage; web build + Playwright test list passed |
| TASK-R7 | done | см. `git log` (`TASK-R7: reduce border-radius and quiet uppercase labels for tool-oriented UI`) | Reduced border-radius on all work surfaces (app-panel 2rem→1rem, subpanels 1.6rem→0.85rem, dashboard/connections cards proportionally); pills/buttons/modals keep rounded-full; .muted-label changed from uppercase+tracking to sentence case 11px font-semibold; web build + Playwright test list passed |
| TASK-R8 | done | см. `git log` (`TASK-R8: add editor keyboard shortcuts — Ctrl+S save, Delete remove node`) | Ctrl+S/Cmd+S calls handleSave with preventDefault; Delete/Backspace on selected node (outside input/textarea/select/contenteditable) opens confirmation dialog; page-level ConfirmationDialog reuses same messages as ConfigPanel delete; web build + Playwright test list passed |
...
| TASK-S3 | done | см. `git log` (`TASK-S3: RU locale visual polish — fix overflow and layout across all pages`) | Audited all pages at 1280px in RU: header, dashboard, connections, editor, executions, login/register. Only issue found: attention filter dropdown options clipped ("Все причины внимания" 20ch → "Любые" 5ch, "Ошибка в последнем запуске" 26ch → "С ошибкой" 9ch, "Активен без запусков" 20ch → "Без запусков" 12ch). All other pages clean; build passes |
