...
## Следующий шаг
TASK-H закрыт. Production deployment `/opt/mini-zapier` уже обновлён до `892bd57`, поэтому следующий практический шаг — вручную открыть editor с реальной PostgreSQL connection и перепроверить visual `DB Query`: список таблиц, выбор `schema.table` и `Тестировать запрос`. Если visual mode всё ещё пустой, следующий кандидат уже вне кода mini-zapier: права пользователя БД или фактическое отсутствие таблиц в доступных schemas. После этой проверки следующий новый TASK — см. backlog.md.
