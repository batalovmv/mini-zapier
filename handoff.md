...
## Текущее состояние
- **Последнее изменение**: TASK-052 — `DB Query metadata introspection + read builder`
- **Статус проекта**: backlog v1 закрыт + post-v1 fix закрыт + TASK-018–052 закрыты
- **Что сделано в TASK-052**:
  - **Backend**: `apps/api/src/connection/introspection.service.ts` — новый IntrospectionService (getTables, getColumns, testQuery), read-only транзакция с statement_timeout, single-statement guard, SELECT/WITH whitelist, row sanitization через JSON round-trip
  - `apps/api/src/connection/dto/test-query.dto.ts` — DTO с class-validator для test query endpoint
  - `apps/api/src/connection/connection.controller.ts` — 3 новых endpoint: GET introspect/tables, GET introspect/tables/:table/columns, POST introspect/query
  - `apps/api/src/connection/connection.service.ts` — expose `getDecryptedCredentials` public method
  - `apps/api/src/connection/connection.module.ts` — register IntrospectionService
  - **Frontend**: `apps/web/src/components/editor/config-forms/DbQueryConfig.tsx` — visual builder с table selector, column checkboxes, filter rows (TemplatedField values), order by, limit, SQL preview, Test button с результатами в таблице. Backward compat: legacy nodes (без _builderState) открываются в Raw SQL mode
  - `apps/web/src/lib/api/introspection.ts` — API-клиент для introspection endpoints
