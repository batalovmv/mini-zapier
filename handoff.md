...
## Следующий шаг
Следующий рабочий срез: `TASK-Q1 — add scalable connections catalog summary API`.

Перед исполнением:
- держать scope только внутри нового summary contract для больших библиотек подключений
- не ломать существующие `GET /connections` и `GET /connections/:id`
- не расширять scope до workflow names list, last used, health check, breaking CRUD changes, RBAC/sharing/OAuth reconnect/tags/bulk actions
...
- `api/shared` build + Swagger/DTO contract
