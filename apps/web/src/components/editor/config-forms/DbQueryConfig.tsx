...
export function DbQueryConfig({
  ...
  const [advancedOpen, setAdvancedOpen] = useState(
    () => !initialBuilderState && hasExistingRawQuery,
  );
  ...
  useEffect(() => {
    ...
    clearTestResults();
  }, [config._builderState, config.query]);
  ...
  useEffect(() => {
    if (mode === 'raw' || metadataUnavailable !== null) {
      setAdvancedOpen(true);
    }
  }, [mode, metadataUnavailable]);
  ...
  function switchToRawEditing() {
    setMode('raw');
    setAdvancedOpen(true);
    builderStateRef.current = undefined;
    onChange((prev) =>
      prev._builderState === undefined
        ? prev
        : { ...prev, _builderState: undefined },
    );
    clearTestResults();
  ...
  const rawQuery = typeof config.query === 'string' ? config.query : '';
  const rawParams = Array.isArray(config.params) ? config.params : [];
  const usingVisualPath = mode === 'visual' && metadataUnavailable === null;
  ...
  const sqlPreview = useMemo(() => {
    if (!usingVisualPath) return '';
    const result = generateSql(builder, tables, knownColumnNames);
    return result?.query ?? '';
  }, [usingVisualPath, builder, tables, knownColumnNames]);
  ...
  const testDisabled = useMemo(() => {
    ...
    if (classifyQuery(rawQuery) === null) return true;
  ...
  const mutationWarning = useMemo(() => {
    if (!usingVisualPath) return null;
    if (
  ...
  }, [usingVisualPath, builder.operation, builder.table, sqlPreview, t]);

  /* ---- Show result as mutation (rowCount only) vs read (table) ---- */
  const isMutationResult = useMemo(() => {
    if (usingVisualPath) return builder.operation !== 'select';
    return classifyQuery(rawQuery) === 'mutation';
  }, [usingVisualPath, builder.operation, rawQuery]);
    ...
    <div className="min-w-0 space-y-4">
      {metadataUnavailable ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50/90 px-3 py-3 text-sm leading-6 text-amber-800 break-words">
          ...
          <p className="font-semibold text-amber-900">{t.rawPathTitle}</p>
