...
export const ru = {
  ...
  stepTest: {
    ...
    sectionDescription: 'Запустите шаг с тестовыми данными для проверки конфигурации.',
    ...
    noConnectionWarning: 'Выберите подключение перед тестом.',
  ...
  configForms: {
    ...
    dbQuery: {
      ...
      testButton: 'Тестировать запрос',
      testRunning: 'Выполняется\u2026',
      testResult: 'Результат',
      testRowCount: (count: number) => `${count} строк(а) возвращено`,
      testError: 'Тест не пройден',
      selectConnectionHint:
        'Выберите PostgreSQL-подключение для визуального конструктора.',
      ...
      addSetValue: 'Добавить значение',
      removeSetValueAriaLabel: 'Удалить значение',
      setValueColumn: 'Колонка',
      setValuePlaceholder: 'Значение',
      testMutationResult: (count: number) =>
        `${count} строк(а) затронуто (откат выполнен)`,
      deleteNoFilterWarning: 'DELETE без фильтров затронет все строки.',
      updateNoFilterWarning: 'UPDATE без фильтров затронет все строки.',
    },
    emailAction: {
      to: 'Кому',
